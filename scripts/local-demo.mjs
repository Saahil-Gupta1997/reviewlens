...
const root = process.cwd(),
  state = path.join(root, ".reviewlens-local"),
  origin = "http://127.0.0.1:8788",
  allowedHosts = new Set(["127.0.0.1:8788", "localhost:8788"]);
await mkdir(state, { recursive: true });
const secretPath = path.join(state, "encryption-secret");
try {
  await writeFile(secretPath, randomBytes(48).toString("base64url"), {
    flag: "wx",
    mode: 0o600,
  });
} catch (e) {
...
async function collect(dir) {
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    else if (entry.name.endsWith(".js")) serverFiles.push(file);
  }
}
...
const modules = [
  serverEntry,
  ...serverFiles.filter((f) => f !== serverEntry),
].map((file) => ({ type: "ESModule", path: file }));
const mf = new Miniflare({
  modules,
  compatibilityDate: "2026-05-01",
  compatibilityFlags: ["nodejs_compat"],
  d1Databases: { DB: "reviewlens-local" },
  d1Persist: path.join(state, "d1"),
  bindings: {
    KEY_ENCRYPTION_SECRET: await readFile(secretPath, "utf8"),
    IDENTITY_GATEWAY: "local-loopback-single-user",
  },
  serviceBindings: { ASSETS: asset },
});
...
for (const entry of journal.entries) {
  ...
  await db.batch([
    ...sql
      ...
      .filter((s) => s.trim())
      .map((s) => db.prepare(s)),
    db.prepare("INSERT INTO local_migrations(name) VALUES(?)").bind(entry.tag),
  ]);
}
const server = createServer(async (req, res) => {
  try {
    ...
    if (!allowedHosts.has(host)) {
      res.writeHead(403);
      res.end("Local demo accepts same-origin loopback requests only.");
      return;
    }
    ...
    if (req.headers.origin) {
      let suppliedOrigin = "";
      try {
        suppliedOrigin = new URL(req.headers.origin).origin;
      } catch {
        res.writeHead(403);
        res.end(
          "Local demo accepts a valid same-origin loopback request only.",
        );
        return;
      }
    ...
    if (!url.pathname.startsWith("/api/") && url.pathname !== "/")
      result = await asset(new Request(url));
    if (!result || result.status === 404) {
      ...
      result = await mf.dispatchFetch(url.href, {
        method: req.method,
        headers,
    ...
    res.writeHead(500);
    res.end("Local demo request failed. Check the terminal and retry.");
  }
});
server.listen(8788, "127.0.0.1", () =>
  console.log(
    `ReviewLens local demo: ${origin}\nSingle user, no login. Do not expose this server to a network. Ctrl+C to stop.`,
  ),
);

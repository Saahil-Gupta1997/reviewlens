...
const { searchEvidence, classifyAspects } = await import('../work/intelligence.mjs');
...
function score(retrieved, relevant) {
  ...
  return {
    ...
    precision: top.length ? hit.length / top.length : relevant.length ? 0 : null,
...
const results = [
  runRetriever('lexical-baseline', (c) => searchEvidence(reviews, c.question, K).map((r) => r.id)),
];

if (capturedHits) {
  results.push(
    runRetriever('on-device-semantic', (c) => {
...
const md = [
  ...
  '',
  '- Generated: ' + stamp,
  '- Golden set: `' + golden.version + '` / corpus ' + reviews.length + ' reviews / k=' + K,
  '- Split: **' + split + '**' + (split === 'holdout' ? ' (release-candidate measurement)' : ''),

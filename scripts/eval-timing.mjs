...
export function capturedTiming(metadata, hitBytes, caseIds) {
  if (!metadata)
    return {
      values: null,
      source: "Not measured: no paired browser timing metadata supplied.",
    };
  if (
    metadata.hits_sha256 !== createHash("sha256").update(hitBytes).digest("hex")
  ) {
  ...
  if (
    ...
    throw Error(
      "Timing metadata requires captured_at, browser/user_agent, and measurement scope.",
    );
  }
  ...
  return {
    values: metadata.search_latency_ms,

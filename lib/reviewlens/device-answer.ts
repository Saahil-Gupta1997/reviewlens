...
export function deviceAnswer(
  ...
  if (!Array.isArray(candidates))
    throw Error("Prepare on-device search and retry.");
  const scoped = filterReviews(reviews, base.filters),
    seen = new Set<string>();
  ...
  return {
    ...base,
    intent: "On-device semantic evidence",
    status: "limited",
    model: "MiniLM-L6-v2 · on-device retrieval",
    summary: valid.length
      ? `Found ${valid.length} potentially relevant passages in ${scoped.length} scoped reviews. Read the sources to judge relevance.`
    ...
    findings: valid.map((h) => ({
      text: "“" + h.quote + "”",
      reviewIds: [h.reviewId],
    })),
    citations: scoped.filter((r) => seen.has(r.id)),
    notes: [
      "This mode FAILED its evaluation and is kept as a measured negative result, not as a working feature. On a labelled golden set it found 0.267 of relevant reviews against a 0.70 bar, and returned evidence for 50% of questions about topics the data does not contain. Prefer Basic analysis.",

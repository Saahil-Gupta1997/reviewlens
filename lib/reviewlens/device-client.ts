...
export class DeviceSearch {
  ...
  cancel() {
    ...
    this.reject?.(
      new DOMException(
        "Operation paused. Saved reviews can resume.",
        "AbortError",
      ),
    );
    this.reject = null;
  ...
  async run(
    action: "status" | "index" | "search" | "clear",
    dataset: string,
    reviews: Review[],
    progress: (value: DeviceProgress) => void = () => {},
    question = "",
    allowedIds: string[] = [],
  ): Promise<{ indexed: number; hits: DeviceHit[] }> {
    if (this.reject)
      throw Error("Another on-device operation is running. Pause it first.");
    if (
      ...
      throw Error(
        "This browser cannot run on-device search. Use Basic analysis or a browser with WebAssembly and site storage.",
      );
    ...
    return new Promise((resolve, reject) => {
      ...
      const resetTimeout = () => {
        ...
        this.timer = setTimeout(() => {
          ...
          reject(
            Error(
              "Model loading or indexing timed out. Retry to resume, or choose Basic analysis.",
            ),
          );
          this.cancel();
        }, 180000);
      ...
      worker.onerror = (event: ErrorEvent) => {
        ...
        reject(
          Error(
            "The on-device worker could not start. Basic analysis remains available. Technical detail: " +
              detail,
          ),
        );
        this.cancel();
      ...
      worker.postMessage({
        ...
        action,
        dataset,
        reviews: reviews.map(({ id, text }) => ({ id, text })),
        question,
        allowedIds,
      });
    });

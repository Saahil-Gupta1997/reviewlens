...
async function recordsFor(dataset) {
  ...
  return new Promise((resolve, reject) => {
    const r = db.transaction('reviews').objectStore('reviews').index('dataset').getAll(dataset);
...
async function write(record) {
  const db = await database();
  return new Promise((resolve, reject) => {
    const tx = db.transaction('reviews', 'readwrite'); tx.objectStore('reviews').put(record);
    tx.oncomplete = resolve; tx.onerror = tx.onabort = () => reject(Error('Device storage is full or unavailable. Clear a local index or use Basic analysis.'));
  });
}
async function clear(dataset) {
  const records = await recordsFor(dataset), db = await database();
...
async function fingerprint(text) {
  const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(INDEX_VERSION + '\n' + text));
...
async function existing(dataset, reviews) {
  ...
  for (const r of reviews) {
    ...
    if (item?.fingerprint === await fingerprint(r.text) && item.passages?.length &&
      item.passages.every(p => validVector(p.vector) && r.text.slice(p.start, p.end) === p.text)) valid.push(item);
  }
  return valid;
}
async function model(progress) {
  if (!extractor) {
    ...
    extractor = await pipeline('feature-extraction', MODEL, {
      ...
      progress_callback: p => {
        if (p.status === 'progress') progress(`Downloading ${p.file}: ${Math.round(p.progress || 0)}%`);
...
self.onmessage = async ({ data }) => {
  ...
  const progress = (message, indexed) => self.postMessage({ id, progress: true, message, indexed });
  try {
    if (action === 'clear') { await clear(dataset); self.postMessage({ id, result: { indexed: 0 } }); return; }
    const saved = await existing(dataset, reviews);
    if (action === 'status') { self.postMessage({ id, result: { indexed: saved.length } }); return; }
    if (action === 'index') {
      const done = new Set(saved.map(x => x.id)); let count = done.size;
      ...
      if (count < reviews.length) {
        ...
        for (const review of reviews) {
          ...
          for (const passage of passages) {
            ...
            if (!validVector(passage.vector)) throw Error('Model returned an invalid embedding. Try rebuilding.');
          }
          await write({ key: dataset + ':' + review.id, dataset, id: review.id, fingerprint: await fingerprint(review.text), passages });
          progress(`Indexed ${++count} of ${reviews.length} reviews on this device.`, count);
        }
      }
      self.postMessage({ id, result: { indexed: count } }); return;
    }
    if (action === 'search') {
      if (saved.length !== reviews.length) throw Error('Build or resume the on-device index in Settings first.');
      const pipe = await model(progress);
      const output = await pipe(question, { pooling: 'mean', normalize: true });
      self.postMessage({ id, result: { hits: rank(Array.from(output.data), saved, allowedIds) } }); return;
    }

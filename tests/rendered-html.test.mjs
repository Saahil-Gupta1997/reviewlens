...
test("production worker renders the ReviewLens shell and serves private API", async () => {
  ...
  const modules = [entry, ...files.filter((file) => file !== entry)].map(
    (file) => ({ type: "ESModule", path: file }),
  );
    ...
    serviceBindings: {
      ASSETS: async (request) => {
        try {
          ...
          return new Response(await readFile(path.join("dist/client", p)));
        } catch {
  ...
  try {
    const r = await mf.dispatchFetch("http://reviewlens.test/", {
      headers: { accept: "text/html" },
    });
    const html = await r.text();
    assert.equal(r.status, 200);
    assert.match(html, /From reviews to reasons/);
    assert.match(html, /ReviewLens/);

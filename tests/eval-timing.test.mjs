...
import { capturedTiming, latencySummary, latencyGate } from '../scripts/eval-timing.mjs';
const bytes = Buffer.from('{"D1":[],"D2":[]}');
const valid = () => ({hits_sha256:createHash('sha256').update(bytes).digest('hex'),captured_at:'2026-09-12T11:07:38Z',browser:'Chrome',measurement:'Worker request round-trip',search_latency_ms:{D1:22,D2:38}});
...
test('missing timing is unmeasured and cannot pass a release gate',()=>{
 ...
 assert.equal(latencyGate(summary.p95_latency_ms,1500).status,'NOT_MEASURED');
...
test('timings must match exact hit bytes and cover every case with finite nonnegative numbers',()=>{
 assert.throws(()=>capturedTiming(valid(),Buffer.from('{}'),['D1']),/SHA-256/);
 for(const bad of [undefined,null,-1,NaN,Infinity,'22']){
  const m=valid();m.search_latency_ms.D1=bad;
  assert.throws(()=>capturedTiming(m,bytes,['D1','D2']),/invalid browser timing/);
 }
 assert.throws(()=>capturedTiming({...valid(),measurement:''},bytes,['D1']),/scope/);
});

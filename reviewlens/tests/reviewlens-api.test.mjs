...
test('ownership blocks reads, answers, and deletions',async()=>{for(const action of [undefined,{action:'ask',datasetId:id,question:'Most common complaints?'},{action:'delete',datasetId:id}])assert.equal((await request(action,'bob',action?'':'?dataset='+id)).status,404);assert.deepEqual((await request(undefined,'bob')).body.datasets,[]);});

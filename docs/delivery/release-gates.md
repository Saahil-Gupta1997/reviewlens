...
| 2   | Formatting            | `npm run format:check` | Any file deviates from the committed Prettier config                         |
| 3   | Lint                  | `npm run lint`         | ESLint error                                                                 |
| 4   | Behaviour             | `npm test` — 36 tests  | Any failure across engine, API, device-ranking and rendered-HTML suites      |
| 5   | Build                 | `npm run build`        | Production build or the rendered-HTML smoke test fails                       |
| 6   | **Retrieval quality** | `npm run eval`         | recall@5 < 0.19, precision@5 < 0.17, absent-topic FP > 0.30, or p95 > 250 ms |

Gate 6 is the one that makes this a release process rather than a test suite. It fails the
...
| --- | -------------------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7   | Semantic release bar | `npm run eval -- --hits <file>` | Only when captured on-device hits exist. Promotes the run to `semantic_release`: recall@5 ≥ 0.70, precision@5 ≥ 0.50, absent-topic FP ≤ 0.25, p95 ≤ 1500 ms |
| 8   | Holdout measurement  | `npm run eval:holdout`          | Release candidates only. Refuses without `--confirm`; appends to `eval/holdout-runs.log`                                                                    |

## Manual — named, not assumed

| #   | Gate                                                         | Owner role   | State                                         |
| --- | ------------------------------------------------------------ | ------------ | --------------------------------------------- |
| 9   | On-device UAT on a real browser                              | QA           | **Not run** — ISS-03, blocks default-on       |
| 10  | Cross-browser IndexedDB lifecycle                            | QA           | **Not run**                                   |
| 11  | Faithfulness review of generated answers                     | ML / Product | **Not run** — ISS-04                          |
| 12  | `IDENTITY_GATEWAY` set for any internet-reachable deployment | Backend      | Enforced in code — API returns 503 without it |

Gates 9–11 are unmet. That is why the on-device feature is

...
| RSK-08 | A recruiter or hiring manager cannot run the demo and forms a judgement from the README alone                    | 5   | 3   | **15** | Mitigate            | Three-minute local path; recorded walkthrough is a P1; hosted demo blocked by ISS-01                                                          | Portfolio owner |
...
| ISS-07 | Semantic retrieval missed its release gate: recall@5 0.267 vs 0.70, absent-topic FP 0.50 vs 0.25; a sweep clears 0 of 66 floors | Cannot be promoted to default-on, and may not be worth its cost | Open, **P0** | Test fp32 to separate quantisation from capacity, then hybrid retrieval, then cut ([result](evaluation-semantic-result.md)) |
| ISS-08 | The worker maps any unrecognised error to "check your connection", reporting a code defect as a network problem | A wrong diagnosis propagated into every project document for three weeks | Open, P1 | Report "could not load" and log the real message |
...
| ISS-06 | Git history is three squashed commits                           | The delivery trail is asserted in documents rather than visible in the repository                                                        | Open                                                                                                 | Work from here lands as branches and PRs; see [program plan](program-plan.md) |
